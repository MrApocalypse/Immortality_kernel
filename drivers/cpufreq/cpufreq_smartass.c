 ...
 *
 *
 *EDITED FOR DragonMODz by ECOTOX
 */
 ...
 * default is 45ms.
 */
#define DEFAULT_RAMP_DOWN_RATE_NS 25000;
...
#define DEFAULT_UP_MIN_FREQ (600*1000);
...
#define DEFAULT_MAX_RAMP_UP (100 * 1000)
...
static unsigned int cpufreq_smartass_calc_freq(unsigned int cpu, struct cpufreq_policy *policy)
{
	...
	//everything is 100mhz steps
	  if(policy->cur <= 1100000 && policy->cur >= 600000) {
/* catch the extra 100mhz gap between 600 and 800 when scaling down -netarchy/ECOTOX */
...
/* catch the gap between 600 and 800 when scaling up -netarchy/ECOTOX */
		if(policy->cur == 600000) {
		      new_freq = policy->cur + 200000;
		      return new_freq;
		}
		else {
